...
<section class="">
    ...
    <div class="auto-rows-fr grid grid-flow-row grid-cols-2 gap-4">
        @foreach ($foods as $food)
            <a href="{{route('food.show', ['id' => $food->id])}}" class="gap-y-2 w-fit flex flex-col pb-4 bg-gray-100">
                <img class="aspect-square w-64 h-64" src="{{ asset('storage/'.$food->images()->first()->path) }}" alt="">
                <div>
                    <h3 class="text-2xl font-extrabold text-center text-gray-800">{{$food->name}}</h3>
                    <h3 class="text-xl font-bold text-center text-gray-800">{{"Rp" . number_format($food->price, 0, '.', '.')}}</h3>
                </div>
            </a>
